...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int)
{
	...
	// ==========================
	// Verificar se a leitura de memória está a funcionar corretamente, se nao funcionar significa que o driver nao esta a ler a memoria do jogo, e sim lixo, o que torna o cheat inutilizavel.
    // ==========================
    std::cout << "\n[+] PID do Jogo: " << Kernel::pid << std::endl;
    std::cout << "[+] Base Address: 0x" << std::hex << base_address << std::dec << std::endl;

    // Tentar ler os 2 primeiros bytes do jogo (Obrigatório ser 'MZ' / 0x5A4D)
    short magic = read<short>(base_address);
    if (magic == 0x5A4D) {
        std::cout << "[+] SUCESSO: A leitura de memoria esta perfeita!" << std::endl;
    }
    else {
        std::cout << "[-] ERRO: A leitura falhou. O driver esta a ler lixo: 0x" << std::hex << magic << std::endl;
    }
